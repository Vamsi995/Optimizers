...
class GradientDescent:
    ...
    def __init__(self, lr, gamma=None, descentMode="SGD"):
        self.weights = np.random.uniform(-5, 5, 2)
        self.descentMode = descentMode
        self.lr = lr
        self.gamma = gamma
    ...
    def SGD_with_momentum(self):
        self.vt = self.pvt * self.gamma + self.lr * (self.find_gradient(self.weights))
        self.weights -= self.vt
        self.pvt = self.vt

    def nesterov_accelerated_grad(self):
    ...
    def animate(self, i):
        ...
        if(self.descentMode == "SGD"):
            self.SGD_update()
        elif(self.descentMode == "NAG"):
            self.nesterov_accelerated_grad()
        elif(self.descentMode == "SGDM"):
